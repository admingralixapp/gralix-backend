...
- CaliShop (`/shop`): Aura Pack gallery (4 bundles: Classic free, Iron Circuit / Zen Garden / Hype Storm at £4.99 each). Each pack pairs a voice tone + ghost skin. Activate packs server-side (`PUT /api/users/me/active-aura`). Purchase via `POST /api/shop/purchase`. Pro signing-bonus modal (`POST /api/shop/claim-free-aura`) unlocks one free paid pack; gated by `hasClaimedSigningBonus`. Promo code input (`POST /api/shop/redeem`): `TESTER2026`=free Pro month, `GHOSTGIFT/ZENGIFT/HYPEGIFT`=free packs. Inventory + activeAura stored in `usersTable` jsonb columns.
...
- Monetization Settings: Subscription section (monthly £14.99/yearly £149.99 toggle, 3-day free trial), Customization section (voice tones + ghost skins at £4.99 each with 4th "Plasma Storm" skin added), Verified Badge toggle (Pro only). Ghost overlay opacity slider in PovReview (localStorage `calicoach_ghost_opacity_v1`).
