...
- CaliShop (`/shop`): Single monetization hub — (1) Purple glassmorphism Pro Subscription card at top (3-day free trial, monthly £14.99/yearly £149.99 toggle, "Why go Pro?" list: AI coaching/verified badge/free aura pack), (2) Pro Signing Bonus banner → claim modal (one free pack for new Pro users), (3) Aura Pack grid (Classic free + Iron Circuit/Zen Garden/Hype Storm at £4.99), (4) Promo code input. Activate packs server-side (`PUT /api/users/me/active-aura`). Purchase via `POST /api/shop/purchase`. Claim bonus via `POST /api/shop/claim-free-aura`. Redeem codes via `POST /api/shop/redeem`: `TESTER2026`=pro month, `GHOSTGIFT/ZENGIFT/HYPEGIFT`=free packs. Inventory + activeAura stored in `usersTable` jsonb.
- Verified Badge: glowing purple `drop-shadow(0 0 5px #a855f7)` ShieldCheck pill — shown on Leaderboard, Community Feed, and Profile pages for users with `showVerifiedBadge=true` (Pro only).
- Settings page contains only: Account Info, Video Storage, Workout (rest duration), Workout Camera & Audio (voice/camera/mirror/calibration), Daily Mobility Reminders, Profile Privacy, and Sign Out. All subscription and customization has moved to the Shop.
- Progress tab is gated behind Pro: free users see the real charts blurred (blur+brightness) with a purple glassmorphism "Premium Analytics" paywall overlay — Crown icon, pitch text, 4 feature bullets, "Start 3-Day Free Trial" CTA → `/shop`. Sidebar shows a purple "🔒 Pro" pill next to "Progress" for free users (desktop); mobile nav shows a purple lock dot on the icon.
