/**
 * Per-exercise mastery system — frontend mirror.
 *
 * Thresholds: 10 / 100 / 500 / 1 000 / 5 000 (reps or seconds, 1:1 ratio).
 */
...
export const TIER_COLORS: Record<MasteryTier, { bg: string; text: string; border: string; glow: string }> = {
  starter:  { bg: "bg-zinc-700",    text: "text-zinc-300",   border: "border-zinc-500",   glow: "" },
  bronze:   { bg: "bg-amber-900/60",text: "text-amber-400",  border: "border-amber-600",  glow: "shadow-amber-500/20" },
  silver:   { bg: "bg-slate-700/60",text: "text-slate-200",  border: "border-slate-400",  glow: "shadow-slate-400/20" },
  gold:     { bg: "bg-yellow-900/60",text: "text-yellow-400",border: "border-yellow-500", glow: "shadow-yellow-500/30" },
  platinum: { bg: "bg-cyan-900/60", text: "text-cyan-300",   border: "border-cyan-400",   glow: "shadow-cyan-400/40" },
};
...
/** Returns the highest tier achieved, or null if below Starter (10). */
...
/** Returns the tier title for a given def and tier. */
...
/** Returns the next tier above current (null if already Platinum). */
export function getNextTier(current: MasteryTier | null): { tier: MasteryTier; value: number } | null {
  if (!current) return MASTERY_THRESHOLDS[0] ?? null;
  const idx = MASTERY_THRESHOLDS.findIndex((t) => t.tier === current);
  return MASTERY_THRESHOLDS[idx + 1] ?? null;
}
